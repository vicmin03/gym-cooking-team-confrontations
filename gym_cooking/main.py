...
def parse_arguments():
    ...
    parser.add_argument("--train", action="store_true", default=False, help="Train madqn agents")
...
def plot_loss_graph(arglist, avg_loss, avg_reward1, avg_reward2):
    ...
    plt.legend()
    plt.savefig('./graphs/rewards_curve_'+arglist.level+str(arglist.num_agents))
    plt.close()
    ...
    plt.legend()
    plt.savefig('./graphs/loss_curve_'+arglist.level+str(arglist.num_agents))
...
if __name__ == '__main__':
    ...
    if arglist.play:
        ...
        TRAINING_STEPS = arglist.training_steps or 5000
        ...
        if train:
            ...
            plot_loss_graph(arglist, avg_loss, avg_reward1, avg_reward2)


        # ---- RUNNING A GAME ---------------------------------
        env.reset()
        for agent in real_agents:
            agent.reset_completed_subtasks()
